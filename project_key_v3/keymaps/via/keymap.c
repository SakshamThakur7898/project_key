...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [LAYER_BASE] = LAYOUT(
        Youtube,   PremierePro,     Photoshop,
        Shapr3d,   Screenshot,        chatgpt,
        cider,   steam,             MO(LAYER_HOLD)
    ),
    [LAYER_HOLD] = LAYOUT(
        TG(LAYER_YOUTUBE),   TG(LAYER_PREMIERE),     TG(LAYER_PHOTOSHOP),
    ...
    [LAYER_YOUTUBE] = LAYOUT(
        KC_J,       KC_K,    KC_L,
        KC_F,     KC_HOME,    KC_M,
        KC_VOLU,       KC_VOLD,   TO(0)
    )
};
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        case Youtube:
            if (record->event.pressed) {
                SEND_STRING(SS_TAP(X_LGUI) SS_DELAY(100) "Brave" SS_DELAY(100) SS_TAP(X_ENT) SS_DELAY(1000) "Youtube.com" SS_TAP(X_ENT));
            }
            break;
        case chatgpt:
            if (record->event.pressed) {
        ...
        case cider:
            if (record->event.pressed) {
                SEND_STRING(SS_TAP(X_LGUI) SS_DELAY(100) "Cider" SS_DELAY(100) SS_TAP(X_ENT));
            }
            break;
        case steam:
            if (record->event.pressed) {
